...
class ScanNetDataset(Dataset, LoadScannetMixin, DataAugmentationMixin):
    def __init__(self, split, cfg):
        # make sure all input params is valid
        # use ground truth for training
        # test can be both ground truth and non-ground truth
        assert cfg.args.pc_type in ['gt', 'pred']
        assert cfg.args.sem_type in ['607']
        assert split in ['train', 'val', 'test']
        if split == 'train':
            cfg.args.pc_type = 'gt'
        ...
        split_scan_ids = set([x.strip() for x in open(split_file, 'r')])
        self.scan_ids = set() # scan ids in data
        json_data = [] # all json data
        self.data = [] # scanrefer data
        
        for anno_type in cfg.get(split, 'train').sources:
            if anno_type == 'anno':
                anno_file = os.path.join(SCAN_FAMILY_BASE, 'annotations/anno.json')
                json_data.extend(json.load(open(anno_file, 'r', encoding='utf-8')))
            elif anno_type == 'referit3d':
                for anno_type in cfg.get(split, 'train').referit3d.anno_type:
                    anno_file = os.path.join(SCAN_FAMILY_BASE,
                                             f'annotations/refer/{anno_type}.jsonl')
                    with jsonlines.open(anno_file, 'r') as _f:
                        for item in _f:
                            if len(item['tokens']) <= 24:
                                json_data.append(item)
                if cfg.get(split, 'train').referit3d.sr3d_plus_aug:
                    anno_file = os.path.join(SCAN_FAMILY_BASE, 'annotations/refer/sr3d+.jsonl')
                    with jsonlines.open(anno_file, 'r') as _f:
                        for item in _f:
                            if len(item['tokens']) <= 24:
                ...
                for anno_type in cfg.get(split, 'train').sgrefer.anno_type:
                    anno_file = os.path.join(SCAN_FAMILY_BASE,
                                             f'annotations/refer/ssg_ref_{anno_type}.json')
                    json_data.extend(json.load(open(anno_file, 'r', encoding='utf-8')))
            elif anno_type == 'sgcaption':
                for anno_type in cfg.get(split, 'train').sgcaption.anno_type:
                    anno_file = os.path.join(SCAN_FAMILY_BASE,
                                             f'annotations/refer/ssg_obj_caption_{anno_type}.json')
                    json_data.extend(json.load(open(anno_file, 'r', encoding='utf-8')))
            else:
                if 'obj_caption' in anno_type:
                    anno_file = os.path.join(SCAN_FAMILY_BASE, f'annotations/ssg_{anno_type}.json')
                else:
                    anno_file = os.path.join(SCAN_FAMILY_BASE, f'annotations/ssg_ref_{anno_type}.json')
                json_data.extend(json.load(open(anno_file, 'r', encoding='utf-8')))
        ...
        # fill parameters
        self.split = split
        self.max_obj_len = cfg.args.max_obj_len - 1
        self.num_points = cfg.args.num_points
        self.pc_type = cfg.args.pc_type
        self.sem_type = cfg.args.sem_type
        self.filter_lang = cfg.args.filter_lang
